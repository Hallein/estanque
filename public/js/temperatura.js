...
var socket = io();
    socket.on('temperatura', function (data) {
        actualizaTemperatura(data);
    });

function actualizaTemperatura(data){
    var elemento = $('#sensor' + data.id);


	var grados_noche = data.temperatura;
	var pixeles_noche = 140 - Math.floor((grados_noche * 2.8));

	var texto_noche = elemento.find('.texto-temperatura');
	texto_noche.text(grados_noche + '°C');
	var mercurio_noche = elemento.find('.d-mercurio');
	mercurio_noche.css('top', pixeles_noche + 'px');

    //elemento.find('.texto-temperatura').text(data.humedad + '%').attr('class', 'porcentaje-hoja').addClass(hoja);
